...
def setup_train_args(parser, omit=None):
    ...
    parser.add_argument(
        "--batch_size", type=int, default=1, help="The batch size to use for training"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--no_progress_bars", action="store_true", help="Do not show any progress bars"
    )
...
def create_train_dataset(args, dataset_args=None):
    gt_extension = (
        args.gt_extension
        if args.gt_extension is not None
        else DataSetType.gt_extension(args.dataset)
    )
...
def run(args):
    ...
    if args.text_generator_params is not None:
        with open(args.text_generator_params, "r") as f:
            args.text_generator_params = json_format.Parse(
                f.read(), TextGeneratorParameters()
            )
    else:
    ...
    if args.line_generator_params is not None:
        with open(args.line_generator_params, "r") as f:
            args.line_generator_params = json_format.Parse(
                f.read(), LineGeneratorParameters()
            )
    else:
    ...
    params.checkpoint_frequency = (
        args.checkpoint_frequency
        if args.checkpoint_frequency >= 0
        else args.early_stopping_frequency
    )
    ...
    params.early_stopping_best_model_output_dir = (
        args.early_stopping_best_model_output_dir
    ...
    for preproc in args.data_preprocessing:
        ...
        pp.type = (
            DataPreprocessorParams.Type.Value(preproc)
    ...
    default_text_normalizer_params(
        params.model.text_preprocessor.children.add(), default=args.text_normalization
    )
    default_text_regularizer_params(
        params.model.text_preprocessor.children.add(), groups=args.text_regularization
    )
    ...
    default_text_normalizer_params(
        params.model.text_postprocessor.children.add(), default=args.text_normalization
    )
    default_text_regularizer_params(
        params.model.text_postprocessor.children.add(), groups=args.text_regularization
    )
