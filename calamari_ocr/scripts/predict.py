...
def run(args):
    ...
    args.extension = (
        args.extension if args.extension else DataSetType.pred_extension(args.dataset)
    )
    ...
    for result, sample in do_prediction:
        ...
        if args.verbose:
            ...
            print(
                "{}: '{}{}{}'".format(
                    sample["id"], lr[get_base_level(sentence)], sentence, "\u202C"
                )
            )
        ...
        dataset.store_text(
            sentence, sample, output_dir=output_dir, extension=args.extension
        )
