...
# def compute_colseps_mconv(args, binary,scale=1.0):
...
def compute_colseps(binary,scale, args):
    ...
    colseps = compute_colseps_conv(args, binary,scale)
    DSAVE("colwsseps",0.7*colseps+0.3*binary, args)
    ...
    if args.maxseps > 0:
        ...
        DSAVE("colseps",0.7*seps+0.3*binary, args)
...
def apply_mask(binary,colseps):
    ...
    DSAVE("masked_seps", masked_seps, args)
...
def compute_line_seeds(binary,bottom,top,colseps,scale, args):
    ...
    DSAVE("lineseeds",[seeds,0.3*tmarked+0.7*bmarked,binary], args)
...
def compute_segmentation(binary,scale, args):
    ...
    DSAVE("seeds",[bottom,top,boxmap], args)
...
def process1(job, args):
    fname,i = job
    global base
    # base,_ = ocrolib.allsplitext('C:\\Users\\Lakshmi.UNIFIEDIT\\Pictures\\1206reults')
    base = os.getcwd()+'\\'+os.path.splitext(os.path.basename(fname))[0]
    outputdir = base
    ...
    segmentation = compute_segmentation(binary,scale, args)
