...
def main():
    ...
    dataset = FileDataSet(
        DataSetMode.TRAIN, images=images, texts=texts, skip_invalid=True
    )
    cross_fold = CrossFold(
        n_folds=args.n_folds, dataset=dataset, output_dir=args.output_dir
    )
    ...
    for fold_id, fold_files in enumerate(cross_fold.folds):
        ...
        for file_id, file in tqdm(
            enumerate(fold_files), total=len(fold_files), desc="Fold {}".format(fold_id)
        ):
            img_file = file
            ...
            output_basename = (
                ...
                else "{:08d}".format(file_id)
            )

            if os.path.exists(img_file) and os.path.exists(txt_file):
                output_file = os.path.join(
                    fold_out_dir, "{}{}".format(output_basename, ext)
                )
                shutil.copyfile(img_file, output_file)

                output_file = os.path.join(
                    fold_out_dir, "{}{}".format(output_basename, ".gt.txt")
                )
