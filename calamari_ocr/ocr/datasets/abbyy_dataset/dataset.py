...
class AbbyyDatasetGenerator(DatasetGenerator):
    ...
    def _load_sample(self, sample, text_only):
        ...
        book = XMLReader(
            [image_path], [xml_path], self.skip_invalid, self.remove_invalid
        ).read()

        if (
            self.mode == DataSetMode.TRAIN
            or self.mode == DataSetMode.PREDICT
...
class AbbyyDataSet(DataSet):
    ...
    def __init__(
        ...
        if len(self.xmlfiles) == 0:
            ...
            self.xmlfiles = [
                split_all_ext(p)[0] + DataSetType.gt_extension(DataSetType.ABBYY)
                for p in files
            ]
        ...
        self.book = XMLReader(
            self.files, self.xmlfiles, skip_invalid, remove_invalid
        ).read()
        self.binary = binary
                    ...
                    self.add_sample(
                        {
                            ...
                            "id": "{}_{}_{}_{}".format(
                                os.path.splitext(
                                    page.xmlFile if page.xmlFile else page.imgFile
                                )[0],
                                p,
                                l,
                                f,
