...
class Checkpoint:
        ...
        while self.version != Checkpoint.VERSION:
            if Checkpoint.VERSION < self.version:
                raise Exception(
                    ...
                    "instance (currently installed: {})".format(
                        self.version, Checkpoint.VERSION, __version__
                    )
                )
            self._single_upgrade()

        print(
            "Successfully upgraded checkpoint version to {}".format(Checkpoint.VERSION)
        )
    ...
    def _single_upgrade(self):
        ...
        if self.version == 0:
            if (
                self.json["model"]["network"]["backend"].get("type", "TENSORFLOW")
                == "TENSORFLOW"
            ):
                tensorflow_rename_variables.rename(
                    self.ckpt_path,
                    "",
                    "",
                    "cnn_lstm/",
