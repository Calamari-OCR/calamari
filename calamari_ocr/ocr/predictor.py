...
class PredictionResult:
    def __init__(
        ...
        self.logits = np.reshape(
            prediction.logits.data, (prediction.logits.rows, prediction.logits.cols)
        )
        self.codec = codec
        ...
        for p in self.prediction.positions:
            ...
            p.global_start = int(
                self.out_to_in_trans.local_to_global(
                    p.local_start, self.data_proc_params
                )
            )
            p.global_end = int(
                self.out_to_in_trans.local_to_global(p.local_end, self.data_proc_params)
            )
            if len(p.chars) > 0:
...
class Predictor:
    def __init__(
        ...
        if checkpoint:
            ...
            self.data_preproc = (
                data_preproc
                if data_preproc
                else data_processor_from_proto(self.model_params.data_preprocessor)
            )
    ...
    def predict_dataset(self, dataset, progress_bar=True, apply_preproc=True):
        ...
        # else:
        input_dataset = StreamingInputDataset(
            dataset,
            self.data_preproc if apply_preproc else None,
    ...
    def predict_raw(
        self, images, progress_bar=True, batch_size=-1, apply_preproc=True, params=None
    ):
        batch_size = (
            len(images)
            if batch_size < 0
            else self.network.batch_size if batch_size == 0 else batch_size
        )
        if apply_preproc:
            images, params = zip(
                *self.data_preproc.apply(
                    images, progress_bar=progress_bar, processes=self.processes
                )
            )
        ...
        for i in range(0, len(images), batch_size):
            ...
            for p, ip in zip(self.network.predict_raw(input_images), input_params):
                yield PredictionResult(
                    ...
                    out_to_in_trans=self.out_to_in_trans,
                    data_proc_params=ip,
    ...
    def predict_input_dataset(self, input_dataset: InputDataset, progress_bar=True):
        ...
        if progress_bar:
            out = tqdm(
                self.network.predict_dataset(input_dataset),
                desc="Prediction",
...
class MultiPredictor:
    def __init__(
        ...
        self.same_preproc = all(
            [
                preproc_params == p.model_params.data_preprocessor
    ...
    def predict_raw(
        ...
        if apply_preproc:
            images, params = zip(
                *self.data_preproc.apply(images, progress_bar=progress_bar)
            )
        else:
            if not params:
    ...
    def predict_dataset(self, dataset, progress_bar=True):
        ...
        print(
            "Prediction of {} models took {}s".format(
                len(self.predictors), time.time() - start_time
            )
        )
