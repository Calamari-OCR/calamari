...
from calamari_ocr.ocr.data_processing.data_preprocessor import (
    DataPreprocessor,
...
from calamari_ocr.ocr.data_processing.scale_to_height_processor import (
    ScaleToHeightProcessor,
)

from calamari_ocr.ocr.data_processing.default_data_preprocessor import (
...
def data_processor_from_proto(data_preprocessor_params):
    if (
        len(data_preprocessor_params.children) > 0
        and data_preprocessor_params.type != DataPreprocessorParams.MULTI_NORMALIZER
    ):
        raise ValueError(
    ...
    raise Exception(
        "Unknown proto type {} of an data processor".format(
            data_preprocessor_params.type
        )
    )
