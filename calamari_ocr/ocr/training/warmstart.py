...
class WarmStarterWithCodecAdaption(WarmStarter):
    ...
    def copy_weights_from_model(
        self, target_model, weights, indices_to_delete, indices_to_add
    ):
        for target_weight, source_weight in zip(target_model.weights, weights):
            ...
            if "kernel" in target_weight.name:
                w_val = np.delete(
                    source_weight, [i - 1 for i in indices_to_delete], axis=1
                )
                ...
                b_val = np.delete(
                    source_weight, [i - 1 for i in indices_to_delete], axis=0
                )
                ...
                raise NotImplementedError(
                    "logits layer is expected to have kernel and bias and nothing else"
                )
