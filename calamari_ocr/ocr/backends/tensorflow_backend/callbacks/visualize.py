...
class VisCallback(keras.callbacks.Callback):
    def __init__(
        ...
        self.loss_stats = RunningStatistics(
            checkpoint_params.stats_size, checkpoint_params.loss_stats
        )
        self.ler_stats = RunningStatistics(
            checkpoint_params.stats_size, checkpoint_params.ler_stats
        )
        self.dt_stats = RunningStatistics(
            checkpoint_params.stats_size, checkpoint_params.dt_stats
        )
    ...
    def on_batch_end(self, batch, logs):
        ...
        if self.display > 0 and self.checkpoint_params.iter % self.display == 0:
            ...
            pred_sentence = self.text_post_proc.apply(
                "".join(self.codec.decode(decoded[0]))
            )
            gt_sentence = self.text_post_proc.apply(
                "".join(self.codec.decode(target[0]))
            )

            self.training_callback.display(
                self.ler_stats.mean(),
    ...
    def _generate(self, count):
        ...
        cer, target, decoded = zip(
            *[self.predict_func(next(it)[0]) for _ in range(count)]
        )
        return (
            np.mean(cer),
            sum(map(sparse_to_lists, target), []),
