...
from calamari_ocr.ocr.text_processing.text_processor import (
    MultiTextProcessor,
    TextProcessor,
    NoopTextProcessor,
)

from calamari_ocr.ocr.text_processing.text_normalizer import (
    TextNormalizer,
    default_text_normalizer_params,
)
from calamari_ocr.ocr.text_processing.text_regularizer import (
    TextRegularizer,
    default_text_regularizer_params,
)
from calamari_ocr.ocr.text_processing.basic_text_processors import (
    StripTextProcessor,
    BidiTextProcessor,
)
from calamari_ocr.ocr.text_processing.default_text_preprocessor import (
    DefaultTextPreprocessor,
)
from calamari_ocr.ocr.text_processing.default_text_postprocessor import (
...
def text_processor_from_proto(text_processor_params, pre_or_post=None):
    if (
        len(text_processor_params.children) > 0
        and text_processor_params.type != TextProcessorParams.MULTI_NORMALIZER
    ):
        raise ValueError(
            "Only a MULTI_NORMALIZER may have children, however got {}".format(
    ...
    if text_processor_params.type == TextProcessorParams.MULTI_NORMALIZER:
        ...
        if not pre_or_post:
            raise Exception(
                "pre or post parameter must be set to specify pre or postprocessing default"
            )
        return {"pre": DefaultTextPreprocessor(), "post": DefaultTextPostprocessor()}[
            pre_or_post.lower()
        ]
    ...
    raise Exception(
        "Unknown proto type {} of an text processor".format(text_processor_params.type)
    )
