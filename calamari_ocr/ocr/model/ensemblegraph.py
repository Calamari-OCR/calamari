...
class EnsembleGraph(GenericGraphBase[ModelParams]):
    def __init__(self, params: ModelParams, name="CalamariGraph", **kwargs):
        ...
        self.fold_graphs = [
            CalamariGraph(params, f"voter_{i}") for i in range(params.ensemble)
        ]
        if self._params.masking_mode > 0:
            logger.warning(
                "Changed masking during training. This should only be used for evaluation!"
            )
    ...
    def build_prediction_graph(self, inputs, training=None):
        ...
        complete_outputs = [
            self.fold_graphs[i].predict(inputs) for i in range(len(self.fold_graphs))
        ]

        lstm_seq_len = complete_outputs[0]["out_len"]  # is the same for all children
        softmax_outputs = tf.stack(
            [out["blank_last_softmax"] for out in complete_outputs], axis=0
        )
    ...
    def build_train_graph(self, inputs, targets, training=None):
        ...
        def training_step():
            ...
            complete_outputs = [
                self.fold_graphs[i].train(inputs, targets)
            ...
            if self._params.masking_mode == 0:
                # Fixed fold ID
                mask = [
                    tf.not_equal(i, targets["fold_id"])
                    for i in range(len(self.fold_graphs))
                ]
                softmax_outputs *= tf.cast(
                    tf.expand_dims(mask, axis=-1), dtype="float32"
                )
        ...
        def validation_step():
            ...
            def gen_empty_output(bs):
                empty = tf.zeros(
                    shape=[bs, max_lstm_seq_len, self._params.classes], dtype="float32"
                )
                return {
            ...
            def apply_single_model(batch):
                ...
                single_batch_data = {
                    k: [tf.gather(v, batch)] for k, v in inputs.items()
                }
                complete_outputs = [
                    tf.cond(
                ...
                paddings = [
                    ([0, 0], [0, max_lstm_seq_len - tf.shape(out["decoded"])[1]])
                ...
                outputs["decoded"] = tf.gather(
                    tf.stack(
                        [
                            tf.pad(
                                out["decoded"], padding, "CONSTANT", constant_values=0
                            )
        ...
        if isinstance(training, bool) or isinstance(training, int):
            blank_last_softmax, lstm_seq_len, complete_outputs = (
                training_step() if training else validation_step()
            )
        else:
