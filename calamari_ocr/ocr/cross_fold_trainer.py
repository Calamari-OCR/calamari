...
def train_individual_model(run_args):
    ...
    for line in run(
        prefix_run_command(
            ...
            args.get("run", None),
            {"threads": args.get("num_threads", -1)},
        ),
...
class CrossFoldTrainer:
    def __init__(
        ...
        self.train_script_path = os.path.abspath(
            os.path.join(this_absdir, "..", "scripts", "train.py")
        )
    ...
    def run(
        ...
        run_args = []
        folds_to_run = (
            single_fold if len(single_fold) > 0 else range(len(cross_fold.folds))
        )
        for fold in folds_to_run:
            ...
            with open(path, "w") as f:
                ...
                fold_args["output_dir"] = os.path.join(
                    temporary_dir, "fold_{}".format(fold)
                )
                fold_args["early_stopping_best_model_output_dir"] = self.best_models_dir
                fold_args["early_stopping_best_model_prefix"] = (
                    self.best_model_label.format(id=fold)
                )
                ...
                if fold_args["weights"]:
                    if (
                        len(fold_args["weights"].strip()) == 0
                        or fold_args["weights"].upper() == "NONE"
                    ):
                        fold_args["weights"] = None
