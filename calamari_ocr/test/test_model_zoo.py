...
from calamari_ocr.scripts.predict_and_eval import main as predict_and_eval_main, PredictAndEvalArgs
    ...
    def test_model_zoo(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            if not os.path.exists('calamari_models'):
                check_call(['sh', '-c', ' '.join(
                    ['wget', '-q', '-O', '-', url, '|', 'tar', 'xz', '&&', 'mv', f'calamari_models-{version}',
                     'calamari_models'])])
            ...
            self.assertLess(full_evaluation['voted']['eval']['avg_ler'], 0.001,
