...
class TestModelMigration(unittest.TestCase):
    ...
    def predict_and_eval(self, model_path):
        ...
        for sample in predictor.predict(file_dataset()):
            self.assertGreater(sample.outputs.avg_char_probability, 0.95)  # The model was trained and should yield good results

    def test_upgrade_from_5(self):
        with tempfile.TemporaryDirectory() as d:
            for filename in {'0.ckpt.h5', '0.ckpt.json'}:
                shutil.copyfile(os.path.join(models_dir, 'version5', filename), os.path.join(d, filename))
    ...
    def test_upgrade_from_4(self):
        with tempfile.TemporaryDirectory() as d:
            for filename in {'0.ckpt.h5', '0.ckpt.json'}:
                shutil.copyfile(os.path.join(models_dir, 'version4', filename), os.path.join(d, filename))
    ...
    def test_upgrade_from_3(self):
        with tempfile.TemporaryDirectory() as d:
            for filename in {'0.ckpt.h5', '0.ckpt.json'}:
                shutil.copyfile(os.path.join(models_dir, 'version3', filename), os.path.join(d, filename))
